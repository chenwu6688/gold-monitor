...
- **数据源**：监测【招行纸黄金(黄金账户)】买卖双价。基准 = 国际现货黄金(伦敦金)换算人民币/克，**直接采用不加校准**（24h 连续交易，闭市后照常跳动）；± `cmb_spread`(默认 2.5) 得到买入价/卖出价双价，一次播报含 基准 + 国内 Au99.99 参考 + 双价 + 两组目标。
...
└──────────────────────────────────────────────────┼───────────────────┘
                 ...
                 │                     │  get_intl_gold_price() │            │
                 │  ┌─────────────┐    │   数据源: 新浪财经     │            │
                 │  │ 伦敦金 XAU/USD│   │    hf_XAU×fx_susdcny  │            │
                 │  │ 现货黄金 24h  │    │    ÷31.1035=元/克    │            │
                 │  └─────────────┘    │  (直接采用,不校准)   │            │
                 │                     │  analyze()             │            │
                 │  └─────────────┘    │   多目标穿越检测        │            │
                 │                     │   两层采样             │            │
...
**数据流一句话**：GitHub 定时器 → `gold_monitor.py --once` → 新浪取伦敦金+汇率换算元/克（直接作为基准）→ ±点差得买卖双价 → 多目标穿越检测 → WxPusher → 个人微信；穿越状态存 `state.json` 提交回仓库持久化。
...
**唯一资产 · 招行纸黄金（黄金账户）**（基准 = 国际现货黄金，24h 追踪）
- **主基准**：国际现货黄金（伦敦金 XAU/USD，新浪 `hf_XAU`）× 美元人民币汇率（新浪 `fx_susdcny`）÷ 31.1035 = 人民币/克，**直接采用不加校准**。**24h 连续交易**，覆盖上金所闭市时段（15:30–21:00、02:30–09:00、周末），解决闭市后基准冻结失真问题。
- **国内参考**：推送显示国内 Au99.99 参考价（闭市时冻结），仅供对比国际与国内口径差异，不参与计算。
- **取数函数**：`get_intl_gold_price()`（新浪伦敦金+汇率换算，主数据源）；`get_cmb_base_price()`（招行公开行情 `api/rate/gold` 取 Au99.99，仅供国内参考行）。
- **口径**：招行纸黄金(黄金账户)无独立公开实时行情接口；程序取国际换算基准 ± 点差得到**买卖双价**：买入价(银行卖你=你的买入成本) = 基准 + `cmb_spread`，卖出价(银行买你=你的卖出回收) = 基准 − `cmb_spread`。实测招行黄金账户点差约 5 元/克（App 显示 买入价 973.18 / 卖出价 968.18），故 `cmb_spread = 5 ÷ 2 = 2.5`。
- **推送内容**：一次播报含 基准（国际现货）+ 国内参考 Au99.99 + 买卖双价 + 两组目标。
- **双向目标监测**：两组独立目标 —— 【买入组】`cmb_target_prices`(默认 880/900) 用**买入价**触发"跌破买入"提醒；【卖出组】`cmb_target_prices_sell`(默认空) 用**卖出价**触发"涨到卖出"提醒。两组各自穿越检测、互不干扰。
- **可配置标签**：`cmb_gold_label`（买入/卖出价前缀，默认"招行黄金"）与 `cmb_account_label`（基准行账户名，默认"黄金账户"）均可配置，对应不同银行的黄金产品。
- **校准**：若 App 显示买卖价与 `基准 ± 2.5` 不符，调 `cmb_spread` 即可（国际口径基准与国内 Au99.99 有约 2 元/克差属正常）。

> 数据源为**免费 + 分钟级延迟**：精度不及付费实时行情，用于趋势感知与到价提醒足够；重大实盘操作以券商/银行终端为准。

### 4.2 多目标穿越检测

- 配置 `cmb_target_prices: [880, 900]`（【买入组】目标，元/克）；卖出组 `cmb_target_prices_sell`（默认空）。
...
| 国际现货(伦敦金)换算基准取数 | ✅ | 新浪 `hf_XAU` × `fx_susdcny` ÷ 31.1035 |
...
- 编辑 `CMB_TARGET_PRICES`（【买入组】目标，用买入价触发跌破提醒）→ 如 `860,880,900`
- 编辑 `CMB_TARGET_PRICES_SELL`（【卖出组】目标，用卖出价触发涨到提醒，如 `1000`；空 = 不监测卖出）
- 编辑 `TARGET_PRICES`（兜底目标，仅当 `CMB_TARGET_PRICES` 未配置时生效）
...
**方式 B（改代码）**：编辑 `config.json` 的 `cmb_target_prices` / `cmb_target_prices_sell` / `cmb_spread` 字段，提交推送（需本机 SSH）。
...
| 金价长时间不变 | 处于休市时段 | 国际现货周末休市(周六04:00-周一07:00)；此前上金所日盘/夜盘时段已全部改为国际基准，仅周末冻结 |
| `state.json` 冲突导致 run 标红 | 本机 push 与定时任务 state 提交冲突 | 不影响监测；可改 state 持久化到分支隔离（见 index.py COS 方案） |
| 取数失败 | 新浪/招行接口变动 / 网络 | 看 Actions 日志 `[warn][黄金账户] 取数失败`；本地 `python3 gold_monitor.py --once` 复现 |
| 招行纸黄金买入/卖出价偏差大 | `cmb_spread` 与实际点差不符 | 对比你招行 App 的买卖价与 `基准 ± cmb_spread`（基准=程序推送的国际现货换算），调 `CMB_SPREAD` / `cmb_spread` |
| 基准与 Au99.99 有约 2 元差 | 正常（国际 vs 国内口径） | 基准=国际现货直接换算，与国内 Au99.99 的差为国际/国内价差，属正常；国内参考行仅供对比 |
| 招行接口偶发 404 / 空 | 招行网关波动 | 仅影响国内参考行（基准仍取国际现货）；程序已内置 3 次重试 + Au99.99→Au(T+D)→Au100g 回退 |
...
| 字段 | 默认值 | 含义 |
|---|---|---|
| `cmb_target_prices` | `[880, 900]` | 【买入组】目标（元/克），缺省沿用 `target_prices` |
| `cmb_target_prices_sell` | `[]` | 【卖出组】目标（元/克）：用卖出价触发"涨到卖出"提醒；空 = 不监测卖出 |
| `target_prices` | `[880, 900]` | 兜底目标（元/克），仅当 `cmb_target_prices` 未配置时生效 |
...
| `cmb_spread` | `2.5` | 招行点差偏移（元/克）：基准 ± 此值 = 招行买卖双价 |
...
| `CMB_TARGET_PRICES` | Variables | `cmb_target_prices`（【买入组】目标，跌破买入提醒） |
| `CMB_TARGET_PRICES_SELL` | Variables（可选） | `cmb_target_prices_sell`（【卖出组】目标，涨到卖出提醒） |
| `TARGET_PRICES` | Variables（可选） | `target_prices`（兜底目标，`CMB_TARGET_PRICES` 未配时生效） |

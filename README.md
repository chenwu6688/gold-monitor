...
1. 拉取上金所 Au99.99 国内实时金价作为基准（即招行纸黄金的中间基准价）
...
- 🎯 **单资产 + 多目标 + 双向监测** — 监测【招行纸黄金(黄金账户)买卖双价】；基准价即上金所 Au99.99（国内金价），一次播报同时含**买入组(跌破买入价提醒 880/900)** 与 **卖出组(涨到卖出价提醒，如 1000)** 两组独立目标
...
### 数据源
**唯一资产 · 招行纸黄金（黄金账户）**（基准 = 上金所 Au99.99 国内金价）
- **接口**：招行公开行情 `https://m.cmbchina.com/api/rate/gold`（无需鉴权，返回 JSON），取上金所 Au99.99 报价为基准
...
- **国内金价合并**：不再单独推送 SGE Au99.99 消息（与招行基准价重复）；招行推送的基准行即国内金价实时价。
- **推送首行**：基准行只显示账户名（`cmb_account_label`，默认 `黄金账户`），如 `- **黄金账户** 基准 Au99.99：968.50 元/克`
...
- **推送内容**：同时展示基准价、买入价与卖出价，一眼看清两边成交价。

...
- **数据源**：监测【招行纸黄金(黄金账户)】买卖双价。程序经招行公开行情接口取上金所 Au99.99（即国内金价）为基准，± `cmb_spread`(默认 2.5) 得到买入价/卖出价双价，一次播报包含国内金价基准 + 招行双价 + 两组目标。不再单独推送 SGE Au99.99 消息（内容重复）。
...
**数据流一句话**：GitHub 定时器 → `gold_monitor.py --once` → 招行公开行情取 Au99.99 基准（=国内金价）→ ±点差得买卖双价 → 多目标穿越检测 → WxPusher → 个人微信；穿越状态 `state.json` 提交回仓库持久化。
...
- **国内金价合并**：Au99.99 基准即国内金价实时价，不再单独推送 SGE Au99.99 消息（与招行基准价重复），一次播报含 基准价 + 买卖双价 + 两组目标。
...
| 招行 Au99.99 基准取数（=国内金价） | ✅ | 招行公开行情 `api/rate/gold` |
...
| 取数失败 | 招行行情接口变动 / 网络 | 看 Actions 日志 `[warn][黄金账户] 取数失败`；本地 `python3 gold_monitor.py --once` 复现 |
